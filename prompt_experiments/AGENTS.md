...
- This folder is dedicated to iterating on the prompt layer for a project that will use Apple's local foundation models `apple_fm_sdk` (macOS only). The prompts and related classes are used for an application that is helping users store and recall facts about their personal life, specifically related to questions related to 'When was the last time I did ...". 

## Classification task
The input is provided to a model, and the objective is to know if the input can be classified into one of three labels: an intent to remember, to recall, or neither.
### Remember
- The user input could be in the form of a statement (e.g Please remember that I went to the bar) or in the form of a request (Can you remember that i went to the bar)
- This statement should always have strong intent about saving (by having obvious command words like 'save', 'log', 'remember', 'note down' etc)
### Recall
- The user input is almost always in the form of a request (Can you tell me when i went to the bar?)
- This statement should always have strong intent about finding out when the user did something?

The model is Apple's Foundation Model. It has a context size of 4096 and completely runs locally on device, and hence requires careful use of the prompt, instructions and guided generation to get the best performance. Keep this in mind when optimising the performance.

## Project setup
- inputs.py : Contains all the prompts, instructions and the corresponding DataClasses used. You are allowed to modify this file.
...
## What to change
You are allowed to heavily iterate the values of the Prompts and Istructions, and change the structure of the Guided generation datastructures (annotated with @fm.generable) to achieve better output. These classes are also used by the foundation model to decide how to generate the output tokens, so make efficient use of all variables at your disposal to come up with the best classification strategy.
...
STOP ONLY WHEN EVALUATION IS ABOVE 90%: Once the experiment loop has begun (after the initial setup), do NOT pause to ask the human if you should continue. Do NOT ask "should I keep going?" or "is this a good stopping point?". The human might be asleep, or gone from a computer and expects you to continue working indefinitely until you are manually stopped. You are autonomous. If you run out of ideas, think harder — look up best prompt practices
